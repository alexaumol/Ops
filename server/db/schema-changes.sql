...
-- 2026-09 — DB-managed email transports (Settings → Email). Replaces the
-- GRAPH_MAIL_* / SMTP_* env vars. Secret columns hold AES-256-GCM ciphertext
-- from server/lib/secrets.js (key: APP_ENCRYPTION_KEY). An entity points at a
-- transport via entity.mail_transport_id; the app-level fallback lives in
-- appconfig 'email.default_transport_id'. See migration
-- 1788518953947_email-transports.sql and docs/email-transports.md.
CREATE TABLE IF NOT EXISTS public.email_transports (
  id            bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS email_transports_name_key ON public.email_transports (lower(name));
ALTER TABLE public.entity
  ADD COLUMN IF NOT EXISTS mail_transport_id bigint REFERENCES public.email_transports(id) ON DELETE SET NULL;
